...
const outputP = Array.from(output.children)
...
function clearCanvas() {
  ...
  for (let number of outputP) {
    number.style.opacity = '0.1'
    number.classList.remove('max-p')
  }
}
...
const updatePredictions = debounce(async () => {
  ...
  for (let i = 0; i < predictions.length; i++) {
    const p = outputP[i]
    p.style.opacity = `${Math.min(Math.max(0.1, predictions[i] * 100), 0.7)}`
    console.log(Math.min(Math.max(0.1, predictions[i]), 0.9))
  
    if (predictions[i] === maxPrediction) {
      p.classList.add('max-p')
    }
  }
}, 100)
